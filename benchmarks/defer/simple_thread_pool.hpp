...
public:
  ...
  public:
    ...
    {
      pool_->post(std::move(f));
    }

    template <class Function>
    void defer(Function f) const
    {
      pool_->defer(std::move(f));
    }
...
private:
  ...
  void run()
  {
    std::unique_ptr<func_base> private_head;
    private_tail() = &private_head;
  ...
  {
    std::unique_ptr<func_base> fp(new func<Function>(std::move(f)));
    if (std::unique_ptr<func_base>*& tail = private_tail())
    {
      *tail = std::move(fp);
      tail = &(*tail)->next_;
    }
  ...
  static std::unique_ptr<func_base>*& private_tail()
  {
    static thread_local std::unique_ptr<func_base>* tail;
    return tail;
  }
