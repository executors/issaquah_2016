// Example of the the actor pattern. Each actor is constructed with a
// runtime-polymorphic executor. This executor must guarantee no concurrent
// upcalls.
...
public:
  ...
  friend void send(Message msg, actor_address from, actor_address to)
  {
    ...
    std::experimental::execution::post(to->executor_,
...
protected:
  ...
  actor(std::experimental::execution::non_blocking_one_way_executor& ex)
    : executor_(ex)
  {
  }
  ...
  {
    // Execute the message handler in the context of the target's executor.
    std::experimental::execution::defer(to->executor_,
...
private:
  ...
  std::experimental::execution::non_blocking_one_way_executor executor_;
...
public:
  receiver()
    : std::experimental::static_thread_pool(1),
      actor(std::experimental::static_thread_pool::executor())
  {
...
{
public:
  explicit member(std::experimental::execution::non_blocking_one_way_executor& ex)
    : actor(ex)
  {
