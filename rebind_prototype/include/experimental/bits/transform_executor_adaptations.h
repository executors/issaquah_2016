#ifndef STD_EXPERIMENTAL_BITS_TRANSFORM_EXECUTOR_ADAPTATIONS_H
#define STD_EXPERIMENTAL_BITS_TRANSFORM_EXECUTOR_ADAPTATIONS_H
...
// Default transform_executor for one way leaves one way executors as is.
...
template<class Executor>
  constexpr typename std::enable_if<
    ...
    && !has_transform_executor_member<Executor, oneway_t>::value, Executor>::type
      transform_executor(Executor ex, oneway_t) { return std::move(ex); }

// Default transform_executor for two way adapts one way executors, leaves two way executors as is.
...
public:
  ...
  twoway_adapter transform_executor(oneway_t) const & { return *this; }
  twoway_adapter transform_executor(oneway_t) && { return std::move(*this); }
  twoway_adapter transform_executor(twoway_t) const & { return *this; }
  twoway_adapter transform_executor(twoway_t) && { return std::move(*this); }

  template<class... T> auto transform_executor(T&&... t) const &
    -> twoway_adapter<typename transform_executor_member_result<InnerExecutor, T...>::type>
      { return { inner_ex_.transform_executor(std::forward<T>(t)...) }; }
  template<class... T> auto transform_executor(T&&... t) &&
    -> twoway_adapter<typename transform_executor_member_result<InnerExecutor&&, T...>::type>
      { return { std::move(inner_ex_).transform_executor(std::forward<T>(t)...) }; }
...
template<class Executor>
  typename std::enable_if<
    ...
    && !has_transform_executor_member<Executor, twoway_t>::value, twoway_adapter<Executor>>::type
      transform_executor(Executor ex, twoway_t) { return twoway_adapter<Executor>(std::move(ex)); }

template<class Executor>
  constexpr typename std::enable_if<
    (is_twoway_executor<Executor>::value || is_bulk_twoway_executor<Executor>::value)
    && !has_transform_executor_member<Executor, twoway_t>::value, Executor>::type
      transform_executor(Executor ex, twoway_t) { return std::move(ex); }

// Default transform_executor for bulk adapts single executors, leaves bulk executors as is.
...
public:
  ...
  bulk_adapter transform_executor(single_t) const & { return *this; }
  bulk_adapter transform_executor(single_t) && { return std::move(*this); }
  bulk_adapter transform_executor(bulk_t) const & { return *this; }
  bulk_adapter transform_executor(bulk_t) && { return std::move(*this); }

  template<class... T> auto transform_executor(T&&... t) const &
    -> bulk_adapter<typename transform_executor_member_result<InnerExecutor, T...>::type>
      { return { inner_ex_.transform_executor(std::forward<T>(t)...) }; }
  template<class... T> auto transform_executor(T&&... t) &&
    -> bulk_adapter<typename transform_executor_member_result<InnerExecutor&&, T...>::type>
      { return { std::move(inner_ex_).transform_executor(std::forward<T>(t)...) }; }
...
template<class Executor>
  typename std::enable_if<is_oneway_executor<Executor>::value
    ...
    && !has_transform_executor_member<Executor, bulk_t>::value, bulk_adapter<Executor>>::type
      transform_executor(Executor ex, bulk_t) { return bulk_adapter<Executor>(std::move(ex)); }

template<class Executor>
  constexpr typename std::enable_if<
    (is_bulk_oneway_executor<Executor>::value || is_bulk_twoway_executor<Executor>::value)
    && !has_transform_executor_member<Executor, bulk_t>::value, Executor>::type
      transform_executor(Executor ex, bulk_t) { return std::move(ex); }

// Default transform_executor for always blocking adapts all executors.
...
public:
  ...
  always_blocking_adapter transform_executor(always_blocking_t) const & { return *this; }
  always_blocking_adapter transform_executor(always_blocking_t) && { return std::move(*this); }
  always_blocking_adapter transform_executor(possibly_blocking_t) const & { return *this; }
  always_blocking_adapter transform_executor(possibly_blocking_t) && { return std::move(*this); }

  template<class... T> auto transform_executor(T&&... t) const &
    -> always_blocking_adapter<typename transform_executor_member_result<InnerExecutor, T...>::type>
      { return { inner_ex_.transform_executor(std::forward<T>(t)...) }; }
  template<class... T> auto transform_executor(T&&... t) &&
    -> always_blocking_adapter<typename transform_executor_member_result<InnerExecutor&&, T...>::type>
      { return { std::move(inner_ex_).transform_executor(std::forward<T>(t)...) }; }
...
template<class Executor>
  constexpr typename std::enable_if<!has_transform_executor_member<Executor, always_blocking_t>::value,
    always_blocking_adapter<Executor>>::type
      transform_executor(Executor ex, always_blocking_t) { return always_blocking_adapter<Executor>(std::move(ex)); }

// Default transform_executor for possibly blocking does no adaptation, as all executors are possibly blocking.

template<class Executor>
  constexpr typename std::enable_if<!has_transform_executor_member<Executor, possibly_blocking_t>::value, Executor>::type
    transform_executor(Executor ex, possibly_blocking_t) { return std::move(ex); }
...
#endif // STD_EXPERIMENTAL_BITS_TRANSFORM_EXECUTOR_ADAPTATIONS_H
