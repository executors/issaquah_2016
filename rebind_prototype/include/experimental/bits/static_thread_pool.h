...
class static_thread_pool
{
  ...
  public:
    ...
    executor_impl transform(execution::oneway_t) const { return *this; }
    executor_impl transform(execution::twoway_t) const { return *this; }

    // Cardinality. Both kinds supported, so requiring does not change type.
    executor_impl transform(execution::single_t) const { return *this; }
    executor_impl transform(execution::bulk_t) const { return *this; }

    // Blocking modes.
    executor_impl<execution::never_blocking_t, Continuation, Work, ProtoAllocator>
      transform(execution::never_blocking_t) const { return {pool_, allocator_}; };
    executor_impl<execution::possibly_blocking_t, Continuation, Work, ProtoAllocator>
      transform(execution::possibly_blocking_t) const { return {pool_, allocator_}; };
    executor_impl<execution::always_blocking_t, Continuation, Work, ProtoAllocator>
      transform(execution::always_blocking_t) const { return {pool_, allocator_}; };

    // Continuation hint.
    executor_impl<Blocking, execution::continuation_t, Work, ProtoAllocator>
      transform(execution::continuation_t) const { return {pool_, allocator_}; };
    executor_impl<Blocking, execution::not_continuation_t, Work, ProtoAllocator>
      transform(execution::not_continuation_t) const { return {pool_, allocator_}; };

    // Work tracking.
    executor_impl<Blocking, Continuation, execution::outstanding_work_t, ProtoAllocator>
      transform(execution::outstanding_work_t) const { return {pool_, allocator_}; };
    executor_impl<Blocking, Continuation, execution::not_outstanding_work_t, ProtoAllocator>
      transform(execution::not_outstanding_work_t) const { return {pool_, allocator_}; };

    // Bulk forward progress.
    executor_impl transform(execution::bulk_parallel_execution_t) const { return *this; }

    // Mapping of execution on to threads.
    executor_impl transform(execution::thread_execution_mapping_t) const { return *this; }

    // Allocator.
    template<class NewProtoAllocator>
    executor_impl<Blocking, Continuation, execution::not_outstanding_work_t, NewProtoAllocator>
      transform(const execution::allocator_t<NewProtoAllocator>& a) const { return {pool_, a.alloc}; };
