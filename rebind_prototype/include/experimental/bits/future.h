...
auto future<R>::then(Executor ex, Function f)
{
  ...
  future_impl::attach(continuation,
      [ex = execution::transform_executor(std::move(ex), execution::oneway), prom = std::move(prom),
        pred = std::move(*this), f = std::move(f)](bool nested_inside_then) mutable
      {
        ...
        if (nested_inside_then)
        {
          ...
          // or execution::try_transform_executor(ex, execution::non_blocking).execute(std::move(func));  // #2: Safe when supported, flexible with effort
          // or execution::transform_executor(ex, execution::non_blocking).execute(std::move(func)); // #3: Safest, least flexible
        }
        else
          execution::try_transform_executor(ex, execution::possibly_blocking).execute(std::move(func));
      });
