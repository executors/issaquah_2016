    ...
    const CompletionExecutor& cex, CompletionHandler h)
{
  if (n == 0)
  {
    ...
    cex.transform(execution::never_blocking).execute(
  ...
  else
  {
    ...
    tex.transform(execution::never_blocking).execute(
        [n, cex = execution::try_transform(cex, execution::outstanding_work), h = std::move(h)]() mutable
        {
          int result = n * 2;
          std::this_thread::sleep_for(std::chrono::seconds(1)); // Simulate long running work.
          execution::try_transform(cex, execution::possibly_blocking).execute(
    ...
    const CompletionExecutor& cex, CompletionHandler h)
{
  ...
  my_twoway_operation_1(tex, n, cex,
    my_twoway_operation_2_impl<decltype(execution::try_transform(tex, execution::continuation)),
      decltype(execution::try_transform(cex, execution::continuation)), CompletionHandler>{
        execution::try_transform(tex, execution::continuation), 0, m,
        execution::try_transform(cex, execution::continuation), std::move(h)});
}
