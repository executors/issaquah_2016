...
public:
  ...
  friend void send(Message msg, actor_address from, actor_address to)
  {
    ...
    std::experimental::execution::transform(to->executor_,
...
protected:
  ...
  {
    // Execute the message handler in the context of the target's executor.
    std::experimental::execution::try_transform(
      std::experimental::execution::transform(to->executor_,
...
int main()
{
  ...
  for (std::size_t i = 0; i < num_actors; ++i)
    members[i] = std::make_shared<member>(pools[(i / actors_per_thread) % num_threads].executor());
