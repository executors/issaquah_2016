...
public:
  static_assert(execution::is_oneway_executor_v<Executor>, "strand transform_executors a one way executor");
  ...
  template <class Property> auto transform_executor(const Property& p) const
    -> strand<execution::transform_executor_member_result_t<Executor, Property>, Blocking>
      { return { state_, ex_.transform_executor(p) }; }

  auto transform_executor(execution::never_blocking_t) const
    -> strand<execution::transform_executor_member_result_t<Executor, execution::never_blocking_t>, execution::never_blocking_t>
  {
    return {state_, ex_.transform_executor(execution::never_blocking)};
  };

  auto transform_executor(execution::possibly_blocking_t) const
    -> strand<execution::transform_executor_member_result_t<Executor, execution::possibly_blocking_t>, execution::never_blocking_t>
  {
    return {state_, ex_.transform_executor(execution::possibly_blocking)};
  };

  void transform_executor(execution::always_blocking_t) const = delete;
  ...
  void execute(Function f) const
  {
    ...
    ex_.execute([s = this->transform_executor(execution::never_blocking).transform_executor(execution::continuation)]() mutable
        {
          s.run_first_item();
        });
  }
...
struct foo
{
  decltype(std::declval<strand<static_thread_pool::executor_type>>().transform_executor(execution::never_blocking)) strand_;
  int count_{0};

  explicit foo(const strand<static_thread_pool::executor_type>& s)
    : strand_(s.transform_executor(execution::never_blocking))
  {
  }
  ...
  void operator()()
  {
    if (count_ < 10)
    {
      ...
      strand_.transform_executor(execution::possibly_blocking).execute([count = count_]{ std::cout << "nested count is " << count << "\n"; });
      ++count_;
...
int main()
{
  ...
  s1.transform_executor(execution::never_blocking).execute(foo{s1});
  s1.transform_executor(execution::possibly_blocking).execute([]{ std::cout << "After 0, before 1\n"; });
  pool.wait();
}
