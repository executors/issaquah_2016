...
namespace custom_hints
{
  ...
  template <class Executor>
    std::enable_if_t<!execution::has_transform_member_v<Executor, tracing>, Executor>
      transform(Executor ex, tracing) { return std::move(ex); }
};
...
int main()
{
  ...
  auto ex2 = execution::transform(pool.executor(), custom_hints::tracing{true});
