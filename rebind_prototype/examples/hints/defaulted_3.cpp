...
namespace custom_hints
{
  ...
  // Default hint implementation creates an adapter, but only when transform() is used.
  ...
  public:
    ...
    // Intercept transform requests for tracing.
    tracing_executor transform(custom_hints::tracing t) const { return { t.on, inner_ex_ }; }

    // Forward other kinds of transform to the inner executor.
    template <class Property> auto transform(const Property& p) const &
      -> tracing_executor<execution::transform_member_result_t<InnerExecutor, Property>>
        { return { tracing_, inner_ex_.transform(p) }; }
    template <class Property> auto transform(const Property& p) &&
      -> tracing_executor<execution::transform_member_result_t<InnerExecutor&&, Property>>
        { return { tracing_, std::move(inner_ex_).transform(p) }; }
  ...
  template <class Executor>
    std::enable_if_t<!execution::has_transform_member_v<Executor, tracing>, tracing_executor<Executor>>
      transform(Executor ex, tracing t) { return { t.on, std::move(ex) }; }

  // This hint cannot be used with try_transform.
  template <class Executor>
    void try_transform(Executor ex, tracing t) = delete;
};
...
int main()
{
  ...
  static_assert(!execution::can_try_transform_v<inline_executor, custom_hints::tracing>, "cannot try_transform");

  auto ex3 = execution::transform(pool.executor(), custom_hints::tracing{true});
  ex3.execute([]{ std::cout << "we made it again\n"; });

  static_assert(!execution::can_try_transform_v<static_thread_pool::executor_type, custom_hints::tracing>, "cannot try_transform");

  execution::executor ex5 = pool.executor();
  auto ex6 = execution::transform(ex5, custom_hints::tracing{true});
  ex6.execute([]{ std::cout << "and again\n"; });

  static_assert(!execution::can_try_transform_v<execution::executor, custom_hints::tracing>, "cannot try_transform");
