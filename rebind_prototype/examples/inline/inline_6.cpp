...
static_assert(execution::is_bulk_twoway_executor_v<decltype(execution::transform(inline_executor(), execution::twoway))>, "bulk two way executor requirements not met");
static_assert(!execution::is_oneway_executor_v<inline_executor>, "must not meet one way executor requirements");
static_assert(!execution::is_twoway_executor_v<decltype(execution::transform(inline_executor(), execution::twoway))>, "must not meet two way executor requirements");

int main()
{
  inline_executor ex1;
  auto ex2 = execution::transform(ex1, execution::twoway);
