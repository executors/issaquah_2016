...
class system_thread_pool_bulk_executor
{
  public:
    ...
    system_thread_pool_bulk_executor transform(execution::bulk_parallel_execution_t) const { return *this; }
  ...
  public:
    ...
    basic_execution_policy<BulkForwardProgressRequirement,OtherExecutor> on(OtherExecutor&& exec) const
    {
      return basic_execution_policy<BulkForwardProgressRequirement,OtherExecutor>(
          execution::transform(std::forward<OtherExecutor>(exec), BulkForwardProgressRequirement{}));
    }
...
void for_each(ExecutionPolicy&& policy, RandomAccessIterator first, RandomAccessIterator last, Function f)
{
  ...
  auto twoway_bulk_exec = execution::transform(policy.executor(), execution::bulk, execution::twoway);
...
public:
  ...
  inline_executor transform(execution::bulk_parallel_execution_t) const
  {
    return *this;
  }
...
int main()
{
  ...
  for_each(par.on(inline_executor()), vec.begin(), vec.end(), [](int& x)
  {
    x = 42;
  });
